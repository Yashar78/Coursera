...
public class TFIDFModelBuilder implements Provider<TFIDFModel> {
    ...
    public TFIDFModel get() {
        ...
        for (long item: items) {
            ...
            List<String > tagList = dao.getItemTags(item);
            Set<Long> seenTags = new HashSet<Long>();
            ...
            for(String tag: tagList ){
            	...
            	if (!work.containsKey(tagKey))
            		work.set(tagKey, 0 );
            	work.add(tagKey, 1);
            	if (!seenTags.contains(tagKey)){
            		seenTags.add(tagKey);
            		docFreq.add(tagKey, 1);
       ...
       // System.out.println(docFreq.toString());
        ...
        for (VectorEntry e: docFreq.fast()) {
        	...
        	docFreq.set(e, Math.log(items.size()/docFreq.get(e)));
        }
       // System.out.println(docFreq.toString());
        //System.exit(0);
