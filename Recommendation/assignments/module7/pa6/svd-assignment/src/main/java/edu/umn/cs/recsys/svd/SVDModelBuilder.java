...
public class SVDModelBuilder implements Provider<SVDModel> {
    ...
    public SVDModel get() {
        ...
        RealMatrix U = svd.getU().getSubMatrix(0, userMapping.size()-1, 0, featureCount-1 );
        RealMatrix VT =  svd.getV().getSubMatrix(0, itemMapping.size()-1, 0, featureCount-1);
        RealMatrix S = svd.getS().getSubMatrix(0, featureCount-1, 0,featureCount-1);
        return new SVDModel(userMapping, itemMapping, U , VT ,S);
    ...
    private RealMatrix createRatingMatrix(IdIndexMapping userMapping, IdIndexMapping itemMapping) {
        ...
        try {
            for (UserHistory<Event> user: users) {
                ...
                //baselines.multiply(-1d);
                //ratings.add(baselines);

                
                for (Long item : ratings.keySet()) {
					int i = itemMapping.getIndex(item);
					//matrix.setEntry(u, i, ratings.get(item)-baselines.get(item));
					matrix.setEntry(u, i, ratings.get(item)-baselineScorer.score(user.getUserId(), item));
					//System.out.println(baselineScorer.score(user.getUserId(), item));
				}
            }
        } finally {
            users.close();
