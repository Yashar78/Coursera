...
object Huffman {
  ...
  def weight(tree: CodeTree): Int =  tree match {
    case Leaf(c,w) => w
    case Fork(l,r,cx,w) => weight(l)+weight(r)
  }

  def chars(tree: CodeTree): List[Char] = tree match {
    case Leaf(c,w) => List(c)
    case Fork(l,r,cx,w) => chars(l) ::: chars(r)
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    ...
    isort(freqs).map(x => new Leaf(x._1,x._2))
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
	def loop(loopTree:CodeTree, loopBits:List[Bit], acc: List[Char]): List[Char] = loopTree match {
		case Fork(lt,rt,cx,w) => loopBits match {
				case List() => acc
				case x :: xs => if (x==1) loop(rt,xs,acc) else loop(lt,xs,acc)
			}
		case Leaf(c,w) =>loopBits match {
			case List() => acc ::: List(c)
			case x :: xs =>  loop(tree,loopBits,acc:::List(c))
...
//    def loop(loopTree: CodeTree,loopText:List[Char], acc:List[Bit]): List[Bit] = 
//    	if (loopText.length==0) acc 
//    	else {
//    	  loopTree match {
//    	    case Leaf(c,w) => acc ::: loop(tree,loopText.tail,acc)
//    	    case Fork(lt,rt,cx,w) => if (lt)
//    	  }
//    	}
//    	  
//      
 ...
 def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
	  def loop(loopTree: CodeTree,loopText:List[Char], acc:List[Bit]): List[Bit] = loopText match {
	    ...
	    case x :: xs =>  loopTree match {
	      case Leaf(c,w) => if (c==x) {
	    	  					if (acc.length==0) 0 :: loop(tree,xs,List()) 
	    	  					else
	    	  						acc ::: loop(tree,xs,List()) 
	    	  					}
	      					else throw new Error("wrong tree")
  ...
  def convert(tree: CodeTree): CodeTable = {
    def loop(loopTree: CodeTree, acc:CodeTable): CodeTable = loopTree match {
    	case Leaf(c,w) => (c,encode(tree)(List(c))) :: acc
    	case Fork(lt,rt,cx,w) => loop(lt,acc) ::: loop(rt,acc)
    }
    loop(tree,List())
  }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def loop(loopText:List[Char], acc:List[Bit]): List[Bit] = loopText match {
    	case List() => acc
    	case x :: xs => loop(xs,acc ::: codeBits(ct)(x))
